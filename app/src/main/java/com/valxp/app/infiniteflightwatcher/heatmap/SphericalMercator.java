...
public class SphericalMercator {
    ...
    public static double yToLatitude(double aY) {
//        return Math.toDegrees(2* Math.atan(Math.exp(Math.toRadians(aY))) - Math.PI/2);
        return toLatitude(aY * 360 - 180);
    }
    ...
    public static double yFromLatitude(double latitude) {
        //return Math.toDegrees(Math.log(Math.tan(Math.PI/4+Math.toRadians(latitude)/2)));
        return (180.0 - fromLatitude(latitude)) / 360.0;
    }
    
    public static double xFromLongitude(double longitude) {
